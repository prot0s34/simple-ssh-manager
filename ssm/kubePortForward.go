...
func startPortForwarding(KubeJumpHost KubeJumpHost) (*exec.Cmd, error) {
	if isPortOpen(KubeJumpHost.LocalPort) {
		log.Printf("Local port %d is already open. Using the existing forwarding...\n", KubeJumpHost.LocalPort)
		return nil, nil
	}

	log.Println("Starting port forwarding...")
	portForwardCmd := exec.Command("kubectl", "port-forward", "svc/"+KubeJumpHost.Service, fmt.Sprintf("%d:%d", KubeJumpHost.LocalPort, KubeJumpHost.ServicePort), "-n", KubeJumpHost.Namespace, "--kubeconfig", KubeJumpHost.KubeconfigPath)
	...
	if !waitForPortOpen(KubeJumpHost.LocalPort, 10*time.Second) {
		portForwardCmd.Process.Kill()
		return nil, fmt.Errorf("timeout reached, port %d did not open", KubeJumpHost.LocalPort)
	}
